...
namespace Gentings.Storages
{
    ...
    public static class HttpClient
    {
        ...
        public static async Task<T> ExecuteAsync<T>(Func<System.Net.Http.HttpClient, Task<T>> func, string referenceUrl = null)
        {
        ...
        /// <param name="referenceUrl">引用地址。</param>
        /// <returns>返回请求得结果实例。</returns>
        public static T Execute<T>(Func<System.Net.Http.HttpClient, T> func, string referenceUrl = null)
        {
            using var client = new System.Net.Http.HttpClient();
            if (referenceUrl != null)
                client.DefaultRequestHeaders.Referrer = new Uri(referenceUrl);
