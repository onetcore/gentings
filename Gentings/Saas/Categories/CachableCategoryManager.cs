...
namespace Gentings.Saas.Categories
{
    /// <summary>
    /// 缓存分类管理实现类基类。
    /// </summary>
        ...
        where TCategory : CategoryBase
    {
        ...
        {
            return Context.Any(x => x.SiteId == category.SiteId && x.Id != category.Id && x.Name == category.Name);
        }
        ...
        /// <returns>返回判断结果。</returns>
        public override Task<bool> IsDuplicatedAsync(TCategory category,
            CancellationToken cancellationToken = default)
        {
            return Context.AnyAsync(x => x.SiteId == category.SiteId && x.Id != category.Id && x.Name == category.Name, cancellationToken);
        }
